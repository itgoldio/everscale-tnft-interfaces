...
IBurnByOwner = 9
ReqInterfaces = [IRequiredInterfaces, IBurnByOwner]

answReqInterfaces = data.call_getter('getRequiredInterfaces')

assert eq(answReqInterfaces, ReqInterfaces)

payload = ts4.encode_message_body('compiled/Data', 'burnByCreator', {})
setcodemultisig.call_method_signed('sendTransaction', {'dest': data.address, 'value': 1000000000, 'bounce': False, 'flags': 0, 'payload': payload}, expect_ec = 0)

ts4.dispatch_messages()

assert eq(None, ts4.get_balance(data.address))